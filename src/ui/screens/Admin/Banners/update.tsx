...
export function UpdateBanner({types, banner}) {
    const {image, setImage, update} = useBanners();
    ...
    useEffect(() => {
        if(banner) {
            type.onChange(banner.bannerType);
            name.setValue(banner.name);
            link.setValue(banner.link);
        }
    }, [banner]);
        ...
        <Container maxWidth="xl">
            ...
            <Box>
                ...
                <Box sx={{
                    ...
                    {banner && banner.path && !image && <Image src={getImageUrl(banner.path)} alt="" width={300} height={300} />}
